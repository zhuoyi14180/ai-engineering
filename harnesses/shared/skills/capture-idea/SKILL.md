...
> **Mode change notice** (show before writing): Once `feature-list.json` exists in the project, the next session will automatically enter **spec-coding mode** — the agent will ask for confirmation before implementing each feature and will not commit automatically. If you only want to capture the idea without changing your current working rhythm, reply `skip` to skip writing.
